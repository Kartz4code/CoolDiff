...
TEST(OpsTest, BasicBinaryOps1) {
  ...
  ASSERT_EQ(result, CoolDiff::TensorR1::Eval(y));
}
...
TEST(OpsTest, BasicBinaryOps2) {
  ...
  ASSERT_EQ(results[0], CoolDiff::TensorR1::DevalF(y, x1));
  ASSERT_EQ(results[1], CoolDiff::TensorR1::DevalF(y, x2));
  ASSERT_EQ(results[2], CoolDiff::TensorR1::DevalF(y, x3));
  ASSERT_EQ(results[3], CoolDiff::TensorR1::DevalF(y, x4));
  ASSERT_EQ(results[4], CoolDiff::TensorR1::DevalF(y, x5));
}
...
TEST(OpsTest, BasicBinaryOps3) {
  ...
  CoolDiff::TensorR1::PreComp(y);
  ASSERT_EQ(results[0], CoolDiff::TensorR1::DevalR(y, x1));
  ASSERT_EQ(results[1], CoolDiff::TensorR1::DevalR(y, x2));
  ASSERT_EQ(results[2], CoolDiff::TensorR1::DevalR(y, x3));
  ASSERT_EQ(results[3], CoolDiff::TensorR1::DevalR(y, x4));
  ASSERT_EQ(results[4], CoolDiff::TensorR1::DevalR(y, x5));
}
...
TEST(OpsTest, BasicBinaryOps4) {
  ...
  // Jacobian evaluation
  for (int i{}; i < 5; ++i) {
    DY[i] = CoolDiff::TensorR1::SymDiff(y, tmp[i]);
    ASSERT_EQ(results[i], CoolDiff::TensorR1::Eval(DY[i]));
  }
}
...
TEST(OpsTest, BasicBinaryOps5) {
  ...
  for (int i{}; i < 5; ++i) {
    for (int j{}; j < 5; ++j) {
      DY[i][j] = CoolDiff::TensorR1::SymDiff(CoolDiff::TensorR1::SymDiff(y, *tmp[i]), *tmp[j]);
      ASSERT_EQ(results[i][j], CoolDiff::TensorR1::Eval(DY[i][j]));
    }
  }
}
...
TEST(OpsTest, BasicBinaryOps6) {
  ...
  for (int i{}; i < 5; ++i) {
    DY[i] = CoolDiff::TensorR1::SymDiff(y, *tmp[i]);
    for (int j{}; j < 5; ++j) {
      ASSERT_EQ(results[i][j], CoolDiff::TensorR1::DevalF(DY[i], *tmp[j]));
    }
  }
}
...
TEST(OpsTest, BasicBinaryOps7) {
  ...
  for (int i{}; i < 5; ++i) {
    DY[i] = CoolDiff::TensorR1::SymDiff(y, *tmp[i]);
    CoolDiff::TensorR1::PreComp(DY[i]);
    for (int j{}; j < 5; ++j) {
      ASSERT_EQ(results[i][j], CoolDiff::TensorR1::DevalR(DY[i], *tmp[j]));
    }
  }
}
...
// Expression evaluation
#if defined(USE_COMPLEX_MATH)
  ASSERT_NEAR(value.real(), CoolDiff::TensorR1::Eval(y).real(), epi);
  ASSERT_NEAR(value.imag(), CoolDiff::TensorR1::Eval(y).imag(), epi);
#else
  ASSERT_NEAR(value, CoolDiff::TensorR1::Eval(y), epi);
#endif
  ...
  for (int i{}; i < 5; ++i) {
    DY[i] = CoolDiff::TensorR1::SymDiff(y, *tmp[i]);
#if defined(USE_COMPLEX_MATH)
    ASSERT_NEAR(results[i].real(), CoolDiff::TensorR1::Eval(DY[i]).real(), epi);
    ASSERT_NEAR(results[i].imag(), CoolDiff::TensorR1::Eval(DY[i]).imag(), epi);
#else
    ASSERT_NEAR(results[i], CoolDiff::TensorR1::Eval(DY[i]), epi);
#endif
  }
}
...
#if defined(USE_COMPLEX_MATH)
      // DevalF
      ASSERT_NEAR(results[i][j].real(), CoolDiff::TensorR1::DevalF(DY[i], *tmp[j]).real(), epi);
      ASSERT_NEAR(results[i][j].imag(), CoolDiff::TensorR1::DevalF(DY[i], *tmp[j]).imag(), epi);
      // DevalR
      ASSERT_NEAR(results[i][j].real(), CoolDiff::TensorR1::DevalR(DY[i], *tmp[j]).real(), epi);
      ASSERT_NEAR(results[i][j].imag(), CoolDiff::TensorR1::DevalR(DY[i], *tmp[j]).imag(), epi);
#else
      ASSERT_NEAR(results[i][j], CoolDiff::TensorR1::DevalF(DY[i], *tmp[j]), epi);
      ASSERT_NEAR(results[i][j], CoolDiff::TensorR1::DevalR(DY[i], *tmp[j]), epi);
#endif
    }
  }
}
...
TEST(OpsTest, LogOps) {
  ...
  Type results[5] = {-14457.8327436, 16298.4626970, 3720.2869560, 1143.5075761,
                     140.6375620};
...
TEST(OpsTest, LogOpsHessian) {
  ...
  Type value = 15237.54114;
  Type results[5][5] = {
      {-12048.1939530, -7228.9163718, -3537.3695631, -1112.1409803,
       -133.8688217},
      {-7228.9163718, 4353.2510736, 3982.9814775, 1252.5164412, 150.6400740},
      {-3537.36956311, 3982.98147749, -14829.02108763, 283.79040213,
       34.40951628},
...
// Expression evaluation
  ...
  for (int i{}; i < 5; ++i) {
    // DevalR evaluation
    Type dr = CoolDiff::TensorR1::DevalR(y, *tmp[i]);
    // DevalR assertion
  ...
  for (int i{}; i < 5; ++i) {
    // DevalF evaluation
    Type df = CoolDiff::TensorR1::DevalF(y, *tmp[i]);
    // DevalF assertion
...
TEST(OpsTest, BinaryTest) {
  ...
  Parameter c{4};
  Expression y = 9 + ProductS(ProductS((x1 + x2), (x3 + ProductS(x4, x5))), 3);
  y = y - ProductS(ProductS(2, x1), c) - 10;
...
TEST(OpsTest, ExpressionCopy) {
  ...
  Type expeval = CoolDiff::TensorR1::Eval(exp);
  Type exp2eval = CoolDiff::TensorR1::Eval(exp2);
  Type exp3eval = CoolDiff::TensorR1::Eval(exp3);

  CoolDiff::TensorR1::PreComp(exp);
  CoolDiff::TensorR1::PreComp(exp2);
  CoolDiff::TensorR1::PreComp(exp3);
  ...
  ASSERT_EQ(CoolDiff::TensorR1::DevalR(expeval, x1), CoolDiff::TensorR1::DevalR(exp2eval, x1));
  ASSERT_EQ(CoolDiff::TensorR1::DevalR(exp2eval, x1), CoolDiff::TensorR1::DevalR(exp3eval, x1));
  ASSERT_EQ(CoolDiff::TensorR1::DevalR(exp3eval, x1), CoolDiff::TensorR1::DevalR(expeval, x1));

  // Reverse derivatives w.r.t. x2
  ASSERT_EQ(CoolDiff::TensorR1::DevalR(expeval, x2), CoolDiff::TensorR1::DevalR(exp2eval, x2));
  ASSERT_EQ(CoolDiff::TensorR1::DevalR(exp2eval, x2), CoolDiff::TensorR1::DevalR(exp3eval, x2));
  ASSERT_EQ(CoolDiff::TensorR1::DevalR(exp3eval, x2), CoolDiff::TensorR1::DevalR(expeval, x2));

  // Forward derivatives w.r.t. x1
  ASSERT_EQ(CoolDiff::TensorR1::DevalF(expeval, x1), CoolDiff::TensorR1::DevalF(exp2eval, x1));
  ASSERT_EQ(CoolDiff::TensorR1::DevalF(exp2eval, x1), CoolDiff::TensorR1::DevalF(exp3eval, x1));
  ASSERT_EQ(CoolDiff::TensorR1::DevalF(exp3eval, x1), CoolDiff::TensorR1::DevalF(expeval, x1));

  // Forward derivatives w.r.t. x2
  ASSERT_EQ(CoolDiff::TensorR1::DevalF(expeval, x2), CoolDiff::TensorR1::DevalF(exp2eval, x2));
  ASSERT_EQ(CoolDiff::TensorR1::DevalF(exp2eval, x2), CoolDiff::TensorR1::DevalF(exp3eval, x2));
  ASSERT_EQ(CoolDiff::TensorR1::DevalF(exp3eval, x2), CoolDiff::TensorR1::DevalF(expeval, x2));
}
