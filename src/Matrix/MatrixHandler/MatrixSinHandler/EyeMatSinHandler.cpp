/**
 * @file src/Matrix/MatrixHandler/MatrixSinHandler/EyeMatSinHandler.cpp
 *
...
void SinEye(const Matrix<Type>* it, Matrix<Type>*& result) {
  /*
  ...
  */
  ...
  std::for_each(EXECUTION_PAR diag_idx.begin(), diag_idx.end(),
                [&](const size_t i) { 
                    (*result)(i, i) = std::sin(1); 
                });
}

void EyeMatSinHandler::handle(const Matrix<Type>* mat, Matrix<Type>*& result) {
  ...
  if (true == IsEyeMatrix(mat)) {
    ...
    // Result matrix is sin identity matrix
    SinEye(mat, result);
    return;
  }
#endif
