/**
 * @file src/Matrix/MatrixHandler/MatrixCos/MatCosNaiveHandler.cpp
 *
...
void MatCosNaiveHandler::handle(const Matrix<Type>* mat, Matrix<Type>*& result) {
  ...
  std::transform(EXECUTION_PAR right, right + size, res, [](const Type a) { return std::cos(a); });
