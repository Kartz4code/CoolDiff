...
class GenericSqrt : public IVariable<GenericSqrt<T, Callables...>>
{
private:
    // Resources
    T *mp_left{nullptr};
...
public:
    ...
    */
    ...
    V_OVERRIDE(Variable *symEval())
    {
        ...
        {
            auto tmp = Allocate<Expression>(sqrt(EVAL_L()));
            this->mp_tmp = tmp.get();
        }
        return this->mp_tmp;
    }
    ...
    V_OVERRIDE(Variable *symDeval(const Variable &var))
    {
        ...
        {
            auto tmp = Allocate<Expression>(((Type)(0.5) / sqrt(EVAL_L())) *
                                            (DEVAL_L(var)));
    ...
    {
        // Return derivative of sqrt: (0.5*pow(u,-0.5))*ud
        const Type du = mp_left->devalF(var);
        const Type u = mp_left->eval();
        return (((Type)(0.5) / std::sqrt(u)) * du);
    }
    ...
    V_OVERRIDE(void traverse(OMPair *cache = nullptr))
    {
        ...
        if (cache == nullptr)
        {
            ...
            const Type u = ((Type)(0.5) / std::sqrt(left));
        ...
        else
        {
            ...
            /* IMPORTANT: The derivative is computed here */
            const Type left = mp_left->eval();
            const Type ustar = (((Type)(0.5) / std::sqrt(left)) * cCache);
...
template <typename T>
const GenericSqrtT<T> &sqrt(const IVariable<T> &u)
{
    auto tmp =
        Allocate<GenericSqrtT<T>>(const_cast<T *>(static_cast<const T *>(&u)),
                                  OpObj);
    return *tmp;
}
