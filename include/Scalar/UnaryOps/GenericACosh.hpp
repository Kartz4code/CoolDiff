...
private:
  ...
  #if 0
    DISABLE_COPY(GenericACosh)
    DISABLE_MOVE(GenericACosh)
  #endif
...
public:
  ...
  // Constructor
  constexpr GenericACosh(T* u, Callables&&... call) : mp_left{u}, 
  ...
  V_OVERRIDE(Type devalF(const Variable& var)) {
    ...
    const Type u = mp_left->eval();
    const Type dr = ((Type)(1) / (std::sqrt((u * u) - (Type)(1))));
    return (dr * du);
  }
  ...
  V_OVERRIDE(void traverse(OMPair* cache = nullptr)) {
    ...
    if (cache == nullptr) {
      ...
      const Type u = ((Type)(1) / (std::sqrt((left * left) - (Type)(1))));
...
constexpr const auto& acosh(const IVariable<T>& u) {
  ...
  auto tmp = Allocate<GenericACoshT<T>>(const_cast<T*>(static_cast<const T*>(&_u)), OpObj);
  return *tmp;
}
