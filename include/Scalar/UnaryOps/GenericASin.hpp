/**
 * @file include/Scalar/UnaryOps/GenericASin.hpp
 *
...
class GenericASin : public IVariable<GenericASin<T, Callables...>> {
 private:
  // Resources
  T *mp_left{nullptr};
  ...
  */
  ...
  V_OVERRIDE(Variable *symEval()) {
    if (nullptr == this->mp_tmp) {
      auto tmp = Allocate<Expression>(asin(EVAL_L()));
      this->mp_tmp = tmp.get();
    }
    return this->mp_tmp;
  }
  ...
  V_OVERRIDE(Type devalF(const Variable &var)) {
    // Return derivative of asin: (1/sqrt(1-(u*u)))*ud
  ...
  V_OVERRIDE(void traverse(OMPair *cache = nullptr)) {
    ...
    if (cache == nullptr) {
      ...
      const Type u = ((Type)(1) / (std::sqrt((Type)(1) - (left * left))));
  ...
  // Get type
  V_OVERRIDE(std::string_view getType() const) { return "GenericASin"; }
...
// Variable asin with 1 typename callables
template <typename T>
using GenericASinT = GenericASin<T, OpType>;

// Function for asin computation
template <typename T>
const GenericASinT<T> &asin(const IVariable<T> &u) {
