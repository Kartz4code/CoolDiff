...
class GenericNeg : public IVariable<GenericNeg<T, Callables...>> {
private:
  // Resources
  T *mp_left{nullptr};
  ...
  DISABLE_COPY(GenericNeg)
  DISABLE_MOVE(GenericNeg)
...
public:
  ...
  // Constructor
  constexpr GenericNeg(T *u, Callables &&...call) : mp_left{u}, 
  ...
  V_OVERRIDE(Variable *symEval()) {
    if (nullptr == this->mp_tmp) {
      auto tmp = Allocate<Expression>((Type)(-1) * (EVAL_L()));
      this->mp_tmp = tmp.get();
    }
    return this->mp_tmp;
  }
  ...
  V_OVERRIDE(Variable *symDeval(const Variable &var)) {
    ...
    if (auto it = this->mp_dtmp.find(var.m_nidx); it == this->mp_dtmp.end()) {
      auto tmp = Allocate<Expression>((Type)(-1) * (DEVAL_L(var)));
  ...
  V_OVERRIDE(void traverse(OMPair *cache = nullptr)) {
    ...
    if (cache == nullptr) {
      ...
      /* IMPORTANT: The derivative is computed here */
      const Type ustar = ((Type)(-1) * cCache);
...
// Generic neg with 1 typename callables
template <typename T> 
using GenericNegT = GenericNeg<T, OpType>;

// Function for neg computation
template <typename T> constexpr const auto &operator-(const IVariable<T> &u) {
  auto tmp = Allocate<GenericNegT<T>>(const_cast<T *>(static_cast<const T *>(&u)), OpObj);
  return *tmp;
}
