/**
 * @file include/Scalar/UnaryOps/GenericATan.hpp
 *
...
class GenericATan : public IVariable<GenericATan<T, Callables...>> {
 private:
  // Resources
  T *mp_left{nullptr};
 ...
 public:
  ...
  // Constructor
  GenericATan(T *u, Callables &&...call)
      : mp_left{u},
  ...
  */
  ...
  V_OVERRIDE(Variable *symEval()) {
    if (nullptr == this->mp_tmp) {
      auto tmp = Allocate<Expression>(atan(EVAL_L()));
      this->mp_tmp = tmp.get();
    }
    return this->mp_tmp;
  }
  ...
  V_OVERRIDE(Variable *symDeval(const Variable &var)) {
    ...
    if (auto it = this->mp_dtmp.find(var.m_nidx); it == this->mp_dtmp.end()) {
      auto tmp = Allocate<Expression>(
          ((Type)(1) / ((Type)(1) + (EVAL_L() * EVAL_L()))) * (DEVAL_L(var)));
  ...
  V_OVERRIDE(Type devalF(const Variable &var)) {
    // Return derivative of atan: (1/(1+(u*u)))*ud
    const Type du = mp_left->devalF(var);
    const Type u = mp_left->eval();
    const Type dr = ((Type)(1) / ((Type)(1) + (u * u)));
    return (dr * du);
  }
  ...
  V_OVERRIDE(void traverse(OMPair *cache = nullptr)) {
    ...
    if (cache == nullptr) {
      ...
      const Type u = ((Type)(1) / ((Type)(1) + (left * left)));
      ...
      /* IMPORTANT: The derivative is computed here */
      const Type left = mp_left->eval();
      const Type ustar = (((Type)1 / ((Type)1 + (left * left))) * cCache);
  ...
  // Get type
  V_OVERRIDE(std::string_view getType() const) { return "GenericATan"; }
...
// Variable atan with 1 typename callables
template <typename T>
using GenericATanT = GenericATan<T, OpType>;

// Function for atan computation
template <typename T>
const GenericATanT<T> &atan(const IVariable<T> &u) {
