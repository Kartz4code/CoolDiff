/**
 * @file include/Scalar/UnaryOps/GenericExp.hpp
 *
...
class GenericExp : public IVariable<GenericExp<T, Callables...>> {
 private:
  // Resources
  T *mp_left{nullptr};
  ...
  */
  ...
  V_OVERRIDE(Variable *symEval()) {
    if (nullptr == this->mp_tmp) {
      auto tmp = Allocate<Expression>(exp(EVAL_L()));
      this->mp_tmp = tmp.get();
    }
    return this->mp_tmp;
  }
  ...
  V_OVERRIDE(Variable *symDeval(const Variable &var)) {
    ...
    if (auto it = this->mp_dtmp.find(var.m_nidx); it == this->mp_dtmp.end()) {
      auto tmp = Allocate<Expression>(exp(EVAL_L()) * (DEVAL_L(var)));
  ...
  V_OVERRIDE(Type devalF(const Variable &var)) {
    // Return derivative of exp: (exp(u))*ud
  ...
  V_OVERRIDE(void traverse(OMPair *cache = nullptr)) {
    ...
    if (cache == nullptr) {
      ...
      const Type u = std::exp(mp_left->eval());
      ...
      /* IMPORTANT: The derivative is computed here */
      const Type ustar = (std::exp(mp_left->eval()) * cCache);
  ...
  // Get type
  V_OVERRIDE(std::string_view getType() const) { return "GenericExp"; }
...
// Generic exp with 1 typename callables
template <typename T>
using GenericExpT = GenericExp<T, OpType>;

// Function for exp computation
template <typename T>
const GenericExpT<T> &exp(const IVariable<T> &u) {
