/**
 * @file include/Scalar/UnaryOps/GenericCos.hpp
 *
...
class GenericCos : public IVariable<GenericCos<T, Callables...>>
{
private:
    // Resources
    T *mp_left{nullptr};
...
public:
    ...
    */
    ...
    V_OVERRIDE(Variable *symEval())
    {
        ...
        {
            auto tmp = Allocate<Expression>(cos(EVAL_L()));
            this->mp_tmp = tmp.get();
        }
        return this->mp_tmp;
    }
    ...
    V_OVERRIDE(Variable *symDeval(const Variable &var))
    {
        ...
        {
            auto tmp = Allocate<Expression>((Type)(-1) * sin(EVAL_L()) *
                                            (DEVAL_L(var)));
    ...
    {
        // Return derivative of cos: -1*(sin(u))*ud
        const Type du = mp_left->devalF(var);
        const Type u = mp_left->eval();
        return ((Type)(-1) * std::sin(u) * du);
    }
    ...
    V_OVERRIDE(void traverse(OMPair *cache = nullptr))
    {
        ...
        if (cache == nullptr)
        {
            ...
            const Type u = (Type)(-1) * std::sin(mp_left->eval());
        ...
        else
        {
            ...
            /* IMPORTANT: The derivative is computed here */
            const Type ustar =
                ((Type)(-1) * std::sin(mp_left->eval()) * cCache);
...
template <typename T>
const GenericCosT<T> &cos(const IVariable<T> &u)
{
    auto tmp =
        Allocate<GenericCosT<T>>(const_cast<T *>(static_cast<const T *>(&u)),
                                 OpObj);
    return *tmp;
}
