/**
 * @file include/Scalar/UnaryOps/GenericACos.hpp
 *
...
public:
    ...
    */
    ...
    V_OVERRIDE(Variable *symEval())
    {
        ...
        {
            auto tmp = Allocate<Expression>(acos(EVAL_L()));
            this->mp_tmp = tmp.get();
        }
        return this->mp_tmp;
    }
    ...
    V_OVERRIDE(void traverse(OMPair *cache = nullptr))
    {
        ...
        if (cache == nullptr)
        {
            ...
            const Type u =
                ((Type)(-1) / (std::sqrt((Type)(1) - (left * left))));
...
{
    auto tmp =
        Allocate<GenericACosT<T>>(const_cast<T *>(static_cast<const T *>(&u)),
                                  OpObj);
    return *tmp;
}