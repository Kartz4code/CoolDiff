/**
 * @file include/MemoryManager/include/MemoryManager/MemoryStrategy/CPUMemoryStrategy/CPUMemoryStrategy.hpp
 *
...
template<typename T>
class CPUMemoryStrategy : public IMemoryStrategy<T> {
    private:
        T* mp_ptr{nullptr}; 
        size_t m_length{}; 

    public:
        CPUMemoryStrategy() = default; 

        // Allocate 
        V_OVERRIDE(void allocate(size_t N)) {
            ASSERT(N > 0, "Length is strictly non-negative");
            m_length = N; mp_ptr = new T[N]{}; 
            ASSERT((nullptr != mp_ptr), "Local CPU allocation failed");
        }
        ...
        V_OVERRIDE(void reset()) {
            if (nullptr != mp_ptr) {
                std::memset(mp_ptr, 0, sizeof(T)*m_length);
            }
        }
        ...
        // Destructor
        V_DTR(~CPUMemoryStrategy()) = default;
};