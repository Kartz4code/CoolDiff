/**
 * @file include/Matrix/MatrixHandler/MatrixHadamardHandler/NaiveCPU/EyeMatHadamardHandler.hpp
 *
...
class EyeMatHadamardHandler : public T {
    public:
      void handle(const Matrix<Type>* lhs, const Matrix<Type>* rhs, Matrix<Type>*& result) {
        ...
        ASSERT((nrows == rrows) && (ncols == lcols), "Matrix Hadamard product dimensions mismatch");
            ...
            if (auto *it = EyeMatHadamard(lhs, rhs); nullptr != it) {
              if (it == lhs || it == rhs) {
                if (-1 == it->getMatType()) {
                  BaselineCPU::HadamardEye(it, result);
                } else {
