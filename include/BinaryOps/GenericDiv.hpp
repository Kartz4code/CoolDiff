/**
 * @file include/BinaryOps/GenericDiv.hpp
 *
...
public:
    ...
    // Constructor
    GenericDiv(T1* u, T2* v, Callables&&... call) : mp_left{ u },
                                                    mp_right{ v },
...
public:
    ...
    // Constructor
    GenericDiv(const Type& u, T* v, Callables&&... call) : mp_left{ u },
                                                           mp_right{ v },
                                                           m_caller{ std::make_tuple(std::forward<Callables>(call)...) },
                                                           m_nidx{ this->m_idx_count++ }
    {}
    ...
    */
    ...
    V_OVERRIDE( Variable* symDeval(const Variable& var) ) {
        ...
        if (auto it = this->mp_dtmp.find(var.m_nidx); it == this->mp_dtmp.end()) {
            ...
            auto tmp = Allocate<Expression>(((Type)(-1) * ((DEVAL_R(var)) * (mp_left))) / 
                                                     ((EVAL_R()*EVAL_R())));
            this->mp_dtmp[var.m_nidx] = tmp.get();
        }
        return this->mp_dtmp[var.m_nidx];
...
template <typename T1, typename T2>
const GenericDivT1<T1, T2>& operator/(const IVariable<T1>& u, const IVariable<T2>& v) {
