...
public:
    ...
    */
    ...
    V_OVERRIDE(Variable *symDeval(const Variable &var))
    {
        ...
        {
            auto tmp =
                Allocate<Expression>(pow(EVAL_L(), EVAL_R()) *
    ...
    */
    ...
    V_OVERRIDE(void traverse(OMPair *cache = nullptr))
    {
        ...
        else
        {
            ...
            const Type v = mp_right->eval();
            const Type ustar =
                (cCache * ((Type)std::pow(mp_left, v) * log(mp_left)));
            (*cache)[mp_right->m_nidx] += (ustar);
...
{
    auto tmp =
        Allocate<GenericPowT3<T>>(const_cast<T *>(static_cast<const T *>(&u)),
                                  v,
                                  OpObj);
    return *tmp;
}
